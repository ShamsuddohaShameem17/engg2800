...
public class SerialController{
  ...
  private static InputStream dataInStream = commPort.getInputStream();
  private static OutputStream dataOutStream = commPort.getOutputStream();
  private SerialPort commPorts[] = SerialPort.getCommPorts();

  byte[] dataSent = new byte[1024];
  byte[] dataRead = new byte[1024];

  
  public void see_available_ports(){
  ...
  public void select_port(String desc){
    ...
    if(!commPort.isOpen()){
      System.out.println("Could not initialize connection to "+desc);
    }

  }
  public void start_connection(SerialPort port){
    commPort.openPort();
